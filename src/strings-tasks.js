...
function countVowels(str) {
  ...
  return str.split('').reduce((a, b) => {
    return vowels.includes(b) ? a + 1 : a;
  }, 0);
}
...
function isPalindrome(str) {
  const newStr = str.replace(/[^a-zA-Z]/gi, '').toLowerCase();
...
function findLongestWord(sentence) {
  const lengths = sentence.split(' ').map((x) => x.length);
  return sentence
    .split(' ')
    .filter((x) => x.length === Math.max(...lengths))[0];
}
  ...
  return str
    ...
    .map((el) => {
      ...
      if (!/[a-zA-Z]/.test(el)) {
        ...
        if (alphabet.indexOf(el) >= 0 && alphabet.indexOf(el) <= 12) {
          a = alphabet[alphabet.indexOf(el) + 13];
        } else if (alphabet.indexOf(el) > 12) {
          a = alphabet[alphabet.indexOf(el) - 13];
        }
      } else if (el !== el.toLowerCase()) {
        if (
          alphabet.indexOf(el.toLowerCase()) + 1 >= 0 &&
          alphabet.indexOf(el.toLowerCase()) <= 12
        ) {
          a = alphabet[alphabet.indexOf(el.toLowerCase()) + 13].toUpperCase();
        } else if (alphabet.indexOf(el.toLowerCase()) > 12) {
          a = alphabet[alphabet.indexOf(el.toLowerCase()) - 13].toUpperCase();
        }
      }
